---
title: ""
pagetitle: "Projects · Andrew Foerder"
page-layout: full
---
...
<div class="page-head">
  <div class="page-head-inner">
    ...
    <p>Three problems: two in imagery, one in statistics. Each
...
<div class="band-narrow">
  ...
  <div class="entry">
    ...
    way those cracks meet can record what the climate did. A crack that stops
    against another suggests the ground cracked in sequence, without repeated
    freezing and thawing. Three cracks meeting symmetrically suggest it froze and
    thawed repeatedly. Cracks that run straight through each other suggest ice
    filled the older fractures before new ones opened.</p>
    ...
    classifies them, labeling 84% correctly overall and 75% when the three junction types
    and the non-junction class are weighted equally, since the rarer types are the hard ones. It needs as few as 25
    ...
    used for paleoclimate interpretation. The pipeline recovers that signal at a scale
    impractical for manual surveys.</p>
    ...
    <p>Training the classifier meant labeling junctions by hand, one at a time, so I
    built a tool for it: the skeleton for context, the crosshair on the node in
    question, four keys for the four classes, and a running count of what was left.</p>

    <video class="demo" controls preload="none" playsinline
           poster="images/labeler-poster.jpg">
      <source src="images/labeler.mp4" type="video/mp4">
      Your browser does not support embedded video.
    </video>
    <p class="figcaption">TYXN Labeler, the annotation tool I built to produce the
    junction training set. Code released with the paper.</p>

    <p class="caveat">The classification method and pilot deployment are under
    review at JGR: Machine Learning and Computation (first submitted April 2026,
    resubmitted July 2026), not yet accepted. The planet-scale deployment is in preparation. Train and test splits
  ...
  <div class="entry">
    ...
    <p class="result">Quantitative results withheld pending publication. The gap these
    models had to cross spans planet, sensor, and resolution at once.</p>
  ...
  <div class="entry">
    ...
    lucky one. Instead, what the record can support is more modest: predicting whether a
    storm already underway will continue to grow.</p>
    ...
    <img src="images/predictability-gap.png" alt="Effect-size axis in sigma showing the largest season-controlled effect at 0.32 with a one-sided 95% upper limit of 0.80, an association floor of 0.59 to 0.81, and an 80%-power detection floor of 1.7 to 1.8 or more">
    ...
    limit. Bands: the effect size this record would need to establish an association, and this
    design would need to detect a precursor at 80% power. Even at the larger, season-uncontrolled
    effect (0.56σ), closing the gap by observation would take at least 222 to 315 Mars years;
    the usable record spans about 6.6.</p>
    ...
    one-sided 95% upper limit of 0.80σ. Even at the larger, season-uncontrolled
    effect (0.56σ), closing that gap by observation would take at least 222 to
    315 Mars years, roughly double that once clustering is counted, and about
    four times more at the season-controlled effect; the usable record spans
    about 6.6. Merely detecting the association, at a season-controlled effect,
    would take roughly 30 to 55 Mars years.</p>

    <p class="caveat">This is a predictability assessment and not a forecasting
    system. Since the project moved to version control on August 3, 2026, methods and
    decision thresholds were committed before each run, so for those runs the
    commit history is the record that nothing was decided after seeing results. One run was killed mid-flight when a check caught rows reading
    atmospheric data from up to about an hour after the forecast was made; no result was read before
    the fix, so the added robustness tests are provably not after the fact. The 80%-power floor is specific to this design; the association floor is a
    property of the record. The nulls exclude
